...
class MLCourseGUI(QMainWindow):
    ...
    def create_data_section(self):
        ...
        # Add loss function selections
        loss_regression_layout = QHBoxLayout()
        loss_regression_layout.addWidget(QLabel("Reg Loss:"))
        loss_regression_layout.addWidget(self.loss_regression_combo)
        data_layout.addLayout(loss_regression_layout)
        
        loss_classification_layout = QHBoxLayout()
        loss_classification_layout.addWidget(QLabel("Class Loss:"))
        loss_classification_layout.addWidget(self.loss_classification_combo)
        data_layout.addLayout(loss_classification_layout)
    ...
    def train_model(self, model_name, param_widgets):
        try:
            ...
            if model_name == "Linear Regression":
                ...
                if len(np.unique(self.y_train)) > 10:  # Regression
                    ...
                    if loss_function == "Cross-Entropy":
                        ...
                        loss = hinge_loss(self.y_test, model.decision_function(self.X_test))
            ...
            if loss is None:
                self.show_error("Loss function not recognized or not applicable.")
                return

            self.current_model = model
            self.update_visualization(y_pred)
            self.update_metrics(y_pred, loss, is_classification)
            self.status_bar.showMessage(f"{model_name} Training Complete. Loss: {loss:.4f}")
    ...
    def update_metrics(self, y_pred, loss, is_classification=True):
